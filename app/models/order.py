...
class Order:
    ...
    def _row_to_dict(row):
        ...
        return {
            ...
            "transaction_code": row[16] if len(row) > 16 else None,
        }
    ...
    def get_all_by_user(mysql, user_id):
        ...
        SELECT id, user_id, customer_name, phone, province, district,
               city, area, address, landmark,
               payment_method, total_price, items_json,
               order_status, payment_status, created_at, transaction_code
        FROM orders
        WHERE user_id = %s
    ...
    def cancel(mysql, order_id, user_id):
        ...
        if order["order_status"] not in ("Pending",):
            return False
        if order.get("transaction_code"):
            # Payment proof already submitted to admin — cancelling now would
            # silently release stock while the admin still believes payment
        ...
        cursor.execute("""
            ...
            WHERE id = %s AND order_status = 'Pending'
              AND user_id = %s AND transaction_code IS NULL
        """, (order_id, user_id))
        mysql.connection.commit()
